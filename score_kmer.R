...
# Fréquence = proportion de patients de la cohorte qui portent la fusion.
per_pat <- patho %>%
...
n_pat_total <- max(n_distinct(per_pat$sample), 1L)
FREQ_LEVELS <- c("≤ 1 %", "1–5 %", "5–15 %", "15–20 %", "> 20 %")
foc <- per_pat %>%
  group_by(gene_pair) %>%
  summarise(max_patient = max(expr, na.rm = TRUE),
            n_pat_pos   = sum(expr > 0), .groups = "drop") %>%
  mutate(freq_pct = 100 * n_pat_pos / n_pat_total,
         freq_bin = factor(cut(freq_pct, breaks = c(-Inf, 1, 5, 15, 20, Inf),
                               labels = FREQ_LEVELS), levels = FREQ_LEVELS))
...
out_cols <- c(
  ...
  "max_patient", "n_pat_pos", "freq_pct", "freq_bin",
...
# palette séquentielle pour la fréquence : rare = clair, fréquent = foncé
FREQ_COLORS <- setNames(c("#fed976", "#feb24c", "#fd8d3c", "#f03b20", "#bd0026"), FREQ_LEVELS)
...
prioris_map <- function(colvar, cscale) {
  p <- ggplot(map_df, aes(x = score_norm, y = max_patient)) +
    ...
    geom_point(aes(color = .data[[colvar]]), size = 3, alpha = 0.85) +   # taille fixe
    scale_y_log10() +
...
if (nrow(map_df) > 0) {
  p_map_freq <- prioris_map("freq_bin",
      scale_color_manual(values = FREQ_COLORS, name = "Fréquence\n(patients positifs)",
                         drop = FALSE, na.value = "grey80")) +
    labs(title = "Carte de priorisation des fusions chromo-spécifiques",
         subtitle = "Score biologique × expression max par patient · couleur = fréquence (patients positifs)")
  ggsave(file.path(DIR_FIG, "carte_priorisation_frequence.pdf"), p_map_freq,
